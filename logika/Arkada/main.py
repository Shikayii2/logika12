...
while not game_over:
    ...
    # Перевірка зіткнення з верхньою стінкою
    if ball.rect.y < 0:
        dy *= -1

    # Перевірка зіткнення з нижньою стінкою (поразка)
    if ball.rect.y > height:
        win_text = Label(150, 150, 50, 50, background_color)
        win_text.set_text("YOU LOSE", 60, (255, 0, 0))
        win_text.draw(10, 10)
        pygame.display.update()
        time.sleep(5)  # Пауза перед завершенням
        game_over = True

    # Перевірка зіткнення з лівою та правою стінками
    ...
    if len(monsters) == 0:
        win_text = Label(150,150,50,50,background_color)
        win_text.set_text("YOU WIN",60,(255,0,0))
        win_text.draw(10,10)
        pygame.display.update()
        time.sleep(5)  # Пауза перед завершенням
        game_over = True
